...
function findElementGreaterThan(arr, num) {
  ...
  if (!Array.isArray(arr)) {
    return undefined; // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо undefined
  }
  ...
  if (foundElement) {
    return foundElement; // Перевіряємо, чи є елемент, який задовольняє умову. Якщо так, повертаємо його.
  } else {
    return "Число, яке задовольняє умову, не знайдено"; // Якщо ні, повертаємо рядок "Число, яке задовольняє умову, не знайдено".
  }
}
...
function findLastEvenNumber(arr) {
  ...
  if (!Array.isArray(arr)) {
    return "Вхідний параметр має бути масивом"; // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо відповідне повідомлення.
  }

  let lastEvenNumber = "Парних чисел не знайдено"; // Ініціалізуємо змінну, яка буде зберігати останнє парне число. Поки що вважаємо, що парних чисел не знайдено.

  for (let i = arr.length - 1; i >= 0; i--) {
    if (arr[i] % 2 === 0) {
      lastEvenNumber = arr[i]; // Знаходимо перше парне число, починаючи з кінця масиву, і присвоюємо його значення змінній lastEvenNumber.
      break; // Зупиняємо цикл після знаходження першого парного числа.
    }
  }

  return lastEvenNumber; // Повертаємо знайдене парне число або повідомлення 'Парних чисел не знайдено'.
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArray = arr.slice(startIdx); // Використовуємо метод slice() для отримання підмасиву від заданого індексу до кінця масиву.
  const subArrayString = subArray.join(" "); // Використовуємо метод join() для конвертації підмасиву в рядок, розділеного пробілами.

  return subArrayString; // Повертаємо рядок, отриманий в результаті конвертації підмасиву.
}
...
function getArrayKeys(arr) {
  ...
  const arrayKeys = Array.from(arr.keys()); // Використовуємо метод keys() для отримання об'єкта ітератора з ключами масиву, а потім конвертуємо його в масив за допомогою методу from().
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = Array.from(arr.values()).filter(
    (value) => value > 0
  ); // Використовуємо метод values() для отримання об'єкта ітератора зі значеннями масиву, а потім конвертуємо його в масив за допомогою методу from(). За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
...
function removeAndDouble(arr, startIdx) {
  ...
  arr.splice(startIdx); // Використовуємо метод splice() для видалення елементів масиву починаючи з заданого індексу.
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flattenedArray = arr.flat(); // Використовуємо метод flat() для вирівнювання масиву.
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const lastElements = arr.slice(-n); // Використовуємо метод slice() для вибору останніх n елементів масиву.
  const sum = lastElements.reduceRight(
    (accumulator, element) => accumulator + element,
    0
  ); // За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.

  lastElements.forEach((element) => console.log(element)); // Виводимо кожен елемент з останніх n в консоль.
...
function wordLengths(arr) {
  ...
  return lengthsArray; // Повертаємо новий масив з довжинами слів.
}
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) || typeof n !== "number") {
    return "Перший вхідний параметр має бути масивом, другий - числом"; // Перевіряємо, чи вхідні параметри є масивом та числом відповідно, якщо ні, повертаємо відповідне повідомлення.
  }
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((longest, current) => {
    if (current.length > longest.length) {
      return current; // Порівнюємо довжину поточного слова з довжиною найдовшого слова. Якщо поточне слово довше, повертаємо його.
    } else {
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((element, index) => {
    ...
    // Якщо індекси різні, але індекс поточного елемента дорівнює індексу останнього входження, то елемент є дубльованим.
  });
